...
# Ensure that all nodes in /dev/mapper correspond to mapped devices currently loaded by the device-mapper kernel driver, adding, changing or removing nodes as necessary.
dmsetup mknodes
...
do
        [ -d $CGROUP_DIR/$SUBSYS ] || mkdir $CGROUP_DIR/$SUBSYS
        mountpoint -q $CGROUP_DIR/$SUBSYS ||
                mount -n -t cgroup -o $SUBSYS cgroup $CGROUP_DIR/$SUBSYS
        ...
        [ $SUBSYS = cpuacct,cpu ] && ln -s $SUBSYS $CGROUP_DIR/cpu,cpuacct
done
